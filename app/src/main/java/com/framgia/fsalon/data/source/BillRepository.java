...
public class BillRepository implements BillDataSource {
    ...
    public Observable<BillResponse> editBill(BillRequest billRequest) {
        return mRemoteDataSource.editBill(billRequest);
    }

    @Override
    public Observable<List<BillResponse>> getBillsByCustomerId(int customerId) {
        return mRemoteDataSource.getBillsByCustomerId(customerId);
    }
}
